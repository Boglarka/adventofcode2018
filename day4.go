...
var input = `[1518-03-19 00:02] Guard #647 begins shift
...
func task1() int {
	guardId, part, part1, part2 := 0, "", 0, 0
	recs := strings.Split(input, "\n")
	alma := make(map[int][]int, 0)
	sort.Sort(Alphabetic(recs))
	for i, rec := range recs {
		if strings.Contains(rec, "#") {
			part = strings.Split(rec, "#")[1]
			guardId, _ = strconv.Atoi(strings.Split(part, " ")[0])
		} else if strings.Contains(rec, "wakes") {
			part1, _ = strconv.Atoi(strings.Split(strings.Split(strings.Split(rec, " ")[1], "]")[0], ":")[1])
			part2, _ = strconv.Atoi(strings.Split(strings.Split(strings.Split(recs[i-1], " ")[1], "]")[0], ":")[1])
			alma[guardId] = append(alma[guardId], part1)
			alma[guardId] = append(alma[guardId], part2)
		}
	}
	sum, max := 0, 0
	guardId = 0
	...
	max = 0
	maxMin, minutes := 0, [60]int{}
	for i := 0; i < len(alma[guardId])-1; i += 2 {
		for j := alma[guardId][i+1]; j < alma[guardId][i]; j++ {
			minutes[j]++
			if minutes[j] > max {
				max = minutes[j]
				maxMin = j
...
func task2() int {
	...
	max := 0
	maxMin, minutes, kj := 0, make(map[string]int), ""
	for k, v := range alma {
		for i := 0; i < len(v)-1; i += 2 {
			for j := v[i+1]; j < v[i]; j++ {
				kj = strconv.Itoa(k) + ":" + strconv.Itoa(j)
				minutes[kj]++
				if minutes[kj] > max {
					max = minutes[kj]
					maxMin = j
